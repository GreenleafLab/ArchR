...
projectBulkATAC <- function(
  ...
  subATAC <- subATAC[order(rowSums(as.array(.getAssay(subATAC, "counts"))), decreasing = TRUE), ]
...
#' Project single cell data into single cell subspace
#' 
#' This function will Project Bulk ATAC-seq data into single cell subspace.
#' 
#' @param projector An `ArchRProject` object containing the dimensionality reduction matrix passed by `reducedDims`.
#' @param projectee a single cell Summarized Experiment containing the cells to be .
...
projectData <- function(
  ...
  .validInput(input = projector, name = "projector", valid = c("ArchRProj"))
  .validInput(input = projectee, name = "projectee", valid = c("SummarizedExperiment"))
  .validInput(input = reducedDims, name = "reducedDims", valid = c("character"))
  .validInput(input = embedding, name = "embedding", valid = c("character", "null"))
  .validInput(input = verbose, name = "verbose", valid = c("boolean"))
  .validInput(input = threads, name = "threads", valid = c("integer"))
  ...
  # Reduced Dimensions
  ##################################################
  rD <- getReducedDims(projector, reducedDims = reducedDims, returnMatrix = FALSE)
  .logThis(names(rD), "reducedDimsNames", logFile = logFile)
  .logThis(rD[[1]], "reducedDimsMat", logFile = logFile)
  rDFeatures <- rD[[grep("Features", names(rD))]]
  if("end" %in% colnames(rDFeatures)){
  ...
  .logThis(rDGR, "reducedDimsGRanges", logFile = logFile)
  subATAC <- subsetByOverlaps(projectee, rDGR, ignore.strand = TRUE)
  subATAC <- subATAC[order(rowSums(as.array(.getAssay(subATAC, "counts"))), decreasing = TRUE), ]
  o <- DataFrame(findOverlaps(subATAC, rDGR, ignore.strand = TRUE))
  sumOverlap <- length(unique(o[,2]))
  .logThis(o, "overlapATAC", logFile = logFile)
  ...
  simRD <- as.matrix(.projectLSI(bulkMat, LSI = rD, verbose = FALSE))
  if(is.null(embedding)){
    if(rD$scaleDims){
      simRD <- .scaleDims(simRD)
    }
  ...
  # Prep Reduced Dims
  ##################################################
  embedding <- getEmbedding(ArchRProj = projector, embedding = embedding, returnDF = FALSE)
  corCutOff <- embedding$params$corCutOff
  dimsToUse <- embedding$params$dimsToUse
  scaleDims <- embedding$params$scaleDims
  ...
  colnames(simUMAP) <- c("UMAP1", "UMAP2")
  dfUMAP <- DataFrame(dfUMAP)
  dfUMAP$Type <- Rle("projector", lengths = nrow(dfUMAP))
