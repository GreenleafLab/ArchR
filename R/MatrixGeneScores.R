...
.addGeneScoreMat <- function(
  ...
  if(!suppressMessages(h5createGroup(file = ArrowFile, "GeneScoreMatrix"))){
    if(force){
      o <- h5delete(file = ArrowFile, name = "GeneScoreMatrix")
      o <- h5createGroup(ArrowFile, "GeneScoreMatrix")
    }else{
      stop("GeneScoreMatrix Already Exists!, set force = TRUE to override!")
    }
  }
  ...
  for(z in seq_along(geneStart)){
    ...
    o <- .addMatToArrow(
      mat = matGS, 
      ArrowFile = ArrowFile, 
      Group = paste0("GeneScoreMatrix/", chri), 
      binarize = FALSE,
      addColSums = TRUE,
      addRowSums = TRUE
      )
