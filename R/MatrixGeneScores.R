...
#' @param geneModel QQQ DOUBLE CHECK. A string giving a "gene model function" used for weighting peaks for gene score calculation. This string should be a function of `x`, where `x` is the stranded distance from the transcription start site of the gene. 
...
#' @param scaleTo QQQ UNCLEAR A numeric value indicating what to scale the computed geneScores to across all cells.
...
#' @param blacklist QQQ WHAT ARE GENE SCORE BIASES? A `GRanges` object containing genomic regions to blacklist for QQQ geneScore biases.
