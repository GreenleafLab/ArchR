...
#' @param trajectory The order of cell groups to be used to constrain supervised fitting (i.e. c("Cluster1", "Cluster2", "Cluster3")).
#' @param groupBy QQQ UNCLEAR initial group column in cellColData to constrain supervised fit
...
#' @param preQuantile QQQ UNCLEAR. WHAT DOES THIS REALLY MEAN? Prior to supervised trajectory fitting, cells whose euclidean distance from the cluster center is QQQ below the provided quantile will be excluded.
#' @param postQuantile QQQ UNCLEAR. WHAT DOES THIS REALLY MEAN? After supervised trajectory fitting, cells whose euclidean distance from the cluster center is QQQ below the provided quantile will be excluded.
...
#' 
#' QQQ UNCLEAR. IMPROVE. This function will get a supervised trajectory from an ArchRProject, aggregate signal
#' from a selected matrix, and smooth QQQ across the trajectory QQQ WHY/FOR WHAT??.
#'
#' @param ArchRProj An `ArchRProject` object.
#' @param name A string indicating the name of the fitted trajectory in `cellColData` to retrieve from the given `ArchRProject`.
#' @param useMatrix QQQ UNCLEAR The name of the data matrix from the `ArrowFiles` to get numerical values for each cell from.
#' @param varCutOff QQQ UNCLEAR WHAT VARIANCE QUANTILE CUTOFF REALLY IS. The "Variance Quantile Cutoff" to be used for identifying the top variable features across the given trajectory.
...
#' @param groupEvery QQQ The number of sequential percentiles to group together when generating a trajectory. This is similar to smoothing via a sliding window. If `groupEvery = 2`, the values for percentiles [0 and 1], [1 and 2], [2 and 3], etc. will be grouped together.
#' @param threads The number of threads to be used for parallel computing.
#' @param scaleTo QQQ UNCLEAR Scale group data matrix to this value for normalizaiton.
#' @param log2Norm A boolean value that indicates whether the summarized trajectory matrix should be log2 transformed.
#' @param smooth QQQ HOW IS THIS DIFFERENT FROM groupEvery?? CLARIFY THE DISTINCTION BECAUSE CURRENTLY GROUPEVERY SAYS "THIS IS SIMILAR TO SMOOTHING". A boolean value indicating whether the summarized trajectory matrix should be smoothed in a row-wise fashion.
...
#' @param labelTop QQQ A number indicating how many of the top N features QQQ based on QQQ in `seTrajectory` should be labeled on the side of the heatmap.
...
#' @param trajectory QQQ WHY DO BOTH NAME AND TRAJECTORY EXIST? ARENT THEY THE SAME? IF NOT, IMPROVE THE DESCRIPTIONS HERE. The column name in `cellColData` that indicates the trajectory to be plotted.
...
#' @param name QQQ WHY DO BOTH NAME AND TRAJECTORY EXIST? ARENT THEY THE SAME? IF NOT, IMPROVE THE DESCRIPTIONS HERE. The name of the trajectory as a column in `cellColData` to plot.
#' @param log2Norm QQQ A boolean value indicating whether a log2 transformation should be performed on the values  QQQ of the trajectory prior to plotting.
...
#' @param quantHex QQQ MISSING INFO HERE
