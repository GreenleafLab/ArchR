...
#' @param positions QQQ ARE LIST AND SIMPLELIST LISTS OF GRANGES?? A `GenomicRangesList`, a `list`, or a `SimpleList` object containing the positions to incorporate into the footprint. Each position should be stranded.
#' @param plotName The prefix to add to the file name for the output PDF file containing the footprint plots.
#' @param groupBy QQQ The name of the column in `cellColData` to use for grouping multiple QQQ cells together prior to footprinting.
...
#' @param nTop QQQ WHAT IF POSITIONS IS NOT A GRANGE? The number of genomic regions to consider. Only the top `nTop` genomic regions based on the "score" column in the `GRanges` object will be considered for the footprint.
#' @param normMethod QQQ The name of the normalization method to use to normalize the footprint relative to the Tn5 insertion bias. Options include "none", "subtract", "divide". QQQ "Subtract" means QQQ. "Divide" means QQQ.
#' @param inputSE QQQ IS THIS STORED IN THE ARCHRPROJECT SOMEWHERE OR AS AN RDS FILE? WHERE WOULD A USER GET THIS? Input a previous footprint Summarized Experiment to be plotted instead of regenerating the footprinting information.
...
#' @param useSink QQQ PLEASE CORRECT EVERYWHERE A boolean value that indicates whether the `sink` function from the QQQ package should be used to hide messages during plotting.
